...
public class RestaurantServiceTest {
    ...
    public void testFindAllRestaurants() {
        ...
        List<Restaurant> restaurantDTOList = restaurantService.findAllRestaurant();

        // Verify the result
        assertEquals(mockRestaurants.size(), restaurantDTOList.size());
        for (int i = 0; i < mockRestaurants.size(); i++) {
            //Restaurant expectedDTO = RestaurantMapper.INSTANCE.mapRestaurantToRestaurantDTO(mockRestaurants.get(i));
            assertEquals(mockRestaurants.get(i), restaurantDTOList.get(i));
        }
    ...
    public void testAddRestaurantInDB() {
        ...
        // Call the service method
        Restaurant savedRestaurantDTO = restaurantService.addRestaurantInDB(mockRestaurant);
    ...
    public void testFetchRestaurantById_ExistingId() {
        ...
        // Call the service method
        ResponseEntity<Restaurant> response = restaurantService.fetchRestaurantById(mockRestaurantId);

        // Verify the response
        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertEquals(Optional.of(mockRestaurantId), response.getBody().getId());
